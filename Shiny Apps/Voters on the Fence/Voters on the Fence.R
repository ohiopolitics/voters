...
server <- function(input, output) 
{
  ...
  filtered_data <- reactive({
    ...
    filtered <- filtered %>%
      ...
      mutate(fence_score = 1-abs(R_probs-D_probs)) %>%
      arrange(desc(fence_score)) %>%
      head(n=ifelse(input$num_results<length(filtered$County),input$num_results,length(filtered$County))) %>%
      select(-c(D_probs,VT_prediction,R_probs))
  })
  ...
  output$download_csv <- downloadHandler(
    filename = function() {
      paste("fence_voters_", Sys.Date(), ".csv", sep = "")
    },
    content = function(file) {
      write.csv(filtered_data() %>%
                  mutate(VoterResponse = ''), file, row.names = FALSE)
    }
  )
}
